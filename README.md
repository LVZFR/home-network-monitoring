...
Internet (Starlink)
 ...
 Starlink Router  ← no admin access, Wi-Fi disabled
        │ (ethernet)
 Asus ZenWiFi XD5 (AX3000)  ← all devices connect here, DHCP DNS → Pi-hole
        │ (ethernet)
 Raspberry Pi 3 (192.168.50.200)  ← Pi-hole DNS server
```
...
| Asus ZenWiFi XD5 (AX3000) | Secondary router, DNS pointed to Pi-hole |
...
Starlink routers are closed systems with no admin panel access. This setup works around that limitation by adding a secondary router (ZenWiFi XD5) where DNS can be configured to point to Pi-hole running on the Raspberry Pi. Every device on the network then has its DNS queries logged and filtered by Pi-hole.
